...
const KEYS_FILE = "/mnt/interview_proxy/interview_keys.json"; // NAS 持久化

function loadKeys() { try { if (fs.existsSync(KEYS_FILE)) return JSON.parse(fs.readFileSync(KEYS_FILE,"utf-8")); } catch {} return {}; }
function saveKeys(keys) { fs.writeFileSync(KEYS_FILE, JSON.stringify(keys,null,2),"utf-8"); }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // 转写 API（自动路由：qwen3-asr-flash → compatible-mode，fun-asr-flash → multimodal）
    if (p === "/api/transcribe" && m === "POST") {
      ...
      // fun-asr-flash 系列 → multimodal-generation 端点
      // qwen3-asr-flash 等旧模型 → compatible-mode 端点
    ...
    // 复盘完成上报（完整一次复盘 = 1 次配额，插件处理完所有步骤后调用）
    if (p === "/api/review-complete" && m === "POST") {
      const k = (headers["x-api-key"]||"").trim();
      if (!k||!keyDb[k]) return json(res, req,401,{error:"Invalid key"});
      if (keyDb[k].used >= keyDb[k].quota) return json(res, req,429,{error:"Quota exhausted"});
